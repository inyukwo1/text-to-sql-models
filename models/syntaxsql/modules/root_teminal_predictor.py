...
class RootTeminalPredictor(nn.Module):
    def __init__(self, H_PARAM, embed_layer, bert=None):
        ...
        self.rt_out_q = nn.Linear(encoded_num, self.N_h)
        self.rt_out_hs = nn.Linear(self.N_h, self.N_h)
        self.rt_out_c = nn.Linear(self.N_h, self.N_h)
        self.rt_out = nn.Sequential(nn.Tanh(), nn.Linear(self.N_h, 2)) #for 2 operators
    ...
    def save_model(self, save_dir):
        print('Saving model...')
        torch.save(self.state_dict(), os.path.join(save_dir, "root_tem_models.dump"))