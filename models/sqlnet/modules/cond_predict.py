...
from commons.utils import run_lstm, col_name_encode
...
class CondPredictor(nn.Module):
    ...
    def forward(self, q_emb_var, q_len, col_emb_var, col_len, col_num, col_name_len, col_bitmap):
        max_q_len = max(q_len)
        ...
        if col_bitmap is not None:
            for idx in range(len(col_len)):
                real_col_num = col_bitmap[idx].nonzero().size(0)
                if real_col_num < len(col_num_score[0]):
                    col_num_score[idx][real_col_num:] = -100
        ...
        # My: set col_score -100 if it belongs to unselected table
        if col_bitmap is not None:
            col_score[col_bitmap != 1.0] = -100
