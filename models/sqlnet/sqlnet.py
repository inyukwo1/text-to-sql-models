...
from models.sqlnet.modules.sel_predict import SelPredictor
from models.sqlnet.modules.cond_predict import CondPredictor
from models.sqlnet.modules.group_predict import GroupPredictor
from models.sqlnet.modules.order_predict import OrderPredictor
...
class SQLNet(nn.Module):
    ...
    def forward(self, input_data):
        # input
        q_seq, col_num, col_org_seq, tbl_ans, from_col_seq, history, q_emb, q_len, q_q_len, sep_embeddings, cols, selected_tbls = input_data

        B = len(q_seq)
        max_col_len = max(col_num)

        concat_q = [list(map(lambda x: ' '.join(x), tmp)) for tmp in q_seq]
        x_emb_var, x_len = self.embed_layer.gen_x_batch(concat_q, is_q=True)
        col_inp_var, col_name_len, col_len = self.embed_layer.gen_col_batch(from_col_seq)
        ...
        sel_score = self.sel_pred(x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len, col_bitmap)
        cond_score = self.cond_pred(x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len, col_bitmap)
        group_score = self.group_pred(x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len, col_bitmap)
        order_score = self.order_pred(x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len, col_bitmap)
    ...
    def loss(self, score, gt_data):
        ...
        if self.use_from:
            ...
            # Add by batch and divide by batch num
            loss += from_loss
    ...
    def preprocess(self, batch):
        ...
        input_data = (q_seq, col_num, col_org_seq, tbl_ans, from_col_seq, history, q_emb, q_len, q_q_len, sep_embeddings, cols, selected_tbls)
