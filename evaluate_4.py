...
def evaluate(gold, predict_list, db_dir, etype, kmaps, table_primarys):
    ...
    extra_hard_rights = []
    for i in range(5):
        easy_rights.append(open("easy_right_{}.txt".format(i), "w"))
        midium_rights.append(open("medium_right_{}.txt".format(i), "w"))
        hard_rights.append(open("hard_right_{}.txt".format(i), "w"))
        extra_hard_rights.append(open("extra_hard_right_{}.txt".format(i), "w"))
    ...
    for q_idx, (p1, p2, p3, p4, g) in enumerate(zip(*plist_list, glist)):
        if q_idx == 32:
            print('here')
        p1_str = p1[0]
        p2_str = p2[0]
        ...
        p1_valid_col_units = build_valid_col_units(p1_sql['from']['table_units'], schema)
        p1_sql = rebuild_sql_val(p1_sql)
        p1_sql = rebuild_sql_col(p1_valid_col_units, p1_sql, kmap)

        p2_valid_col_units = build_valid_col_units(p2_sql['from']['table_units'], schema)
        p2_sql = rebuild_sql_val(p2_sql)
        p2_sql = rebuild_sql_col(p2_valid_col_units, p2_sql, kmap)

        p3_valid_col_units = build_valid_col_units(p3_sql['from']['table_units'], schema)
        p3_sql = rebuild_sql_val(p3_sql)
        p3_sql = rebuild_sql_col(p3_valid_col_units, p3_sql, kmap)

        p4_valid_col_units = build_valid_col_units(p4_sql['from']['table_units'], schema)
        p4_sql = rebuild_sql_val(p4_sql)
        p4_sql = rebuild_sql_col(p4_valid_col_units, p4_sql, kmap)
        ...
        if etype in ["all", "match"]:
            score_1 = evaluator.eval_exact_match(p1_sql, deepcopy(g_sql))
            score_2 = evaluator.eval_exact_match(p2_sql, deepcopy(g_sql))
            score_3 = evaluator.eval_exact_match(p3_sql, deepcopy(g_sql))
            score_4 = evaluator.eval_exact_match(p4_sql, deepcopy(g_sql))

            total_score = score_1 + score_2 + score_3 + score_4
            if hardness == "easy":
                write_file = easy_rights[total_score]
            elif hardness == "medium":
                write_file = midium_rights[total_score]
            elif hardness == "hard":
                write_file = hard_rights[total_score]
            elif hardness == "extra":
                write_file = extra_hard_rights[total_score]

            write_file.write("QUERY ID: {}\n".format(q_idx))
            write_file.write("DB ID: {}\n".format(dev_json[q_idx]["db_id"]))
            write_file.write("NLQ: {}\n".format(dev_json[q_idx]["question"]))
            write_file.write("GOLD: {}\n".format(g_str))
            if score_1 == 0:
                write_file.write("WRONG PREDICTIONS(model 0): {}\n".format(p1_str))
            if score_2 == 0:
                write_file.write("WRONG PREDICTIONS(model 1): {}\n".format(p2_str))
            if score_3 == 0:
                write_file.write("WRONG PREDICTIONS(model 2): {}\n".format(p3_str))
            if score_4 == 0:
                write_file.write("WRONG PREDICTIONS(model 3): {}\n".format(p4_str))
            write_file.write("\n")
    tmp_f.close()
...
if __name__ == "__main__":
    ...
    pred = ["default1.txt", "default2.txt", "default3.txt", "default4.txt"]
    db_dir = args.db
    table = args.table
    etype = args.etype
