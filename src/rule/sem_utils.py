...
def alter_not_in(datas, schemas):
    for d in datas:
        alter_not_in_one_entry(d, schemas[d['db_id']])


def alter_not_in_one_entry(d, current_table):
...
def alter_column0(datas):
    ...
    result = []
    for d in datas:
        z, c, r = alter_column0_one_entry(d)
        zero_count += z
        count += c
        result += result
...
def alter_column0_one_entry(d):
    ...
    return zero_count, count, result